...
async function initApplication() {
    ...
    // @perf Hide first-scan latency on capable mobile/desktop browsers while
    // respecting data-saver, very slow networks, and low-memory phones.
    if (Utils.shouldWarmOcr()) {
        const warmOcr = () => { Receipt.warmEngine(); };
        if (typeof requestIdleCallback === 'function') {
            requestIdleCallback(warmOcr, { timeout: OCR_CONFIG.performance.idleWarmTimeoutMs });
        } else {
            setTimeout(warmOcr, OCR_CONFIG.performance.fallbackWarmDelayMs);
        }
    }
