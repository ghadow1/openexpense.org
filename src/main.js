import { CODE_TAGS, CONFIG, OCR_CONFIG, STORAGE_KEYS } from './config.js';
...
async function initApplication() {
    ...
    const warmOcr = () => {
        document.body.dataset.ocrWarmupTag = CODE_TAGS.app.idleOcrWarmup;
        if (Utils.shouldWarmOcr()) Receipt.warmEngine();
    };
    if (typeof requestIdleCallback === 'function') {
        requestIdleCallback(warmOcr, { timeout: OCR_CONFIG.warmup.requestIdleTimeoutMs });
    } else {
