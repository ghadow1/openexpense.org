...
async function initApplication() {
    ...
    // @ocr-engine Warm opportunistically on capable sessions; constrained
    // devices still load OCR lazily when the user starts a scan.
    if (Utils.shouldWarmOcr()) {
        const warmOcr = () => { Receipt.warmEngine(); };
        if (typeof requestIdleCallback === 'function') {
            requestIdleCallback(warmOcr, { timeout: OCR_CONFIG.performance.warmupIdleTimeoutMs });
        } else {
            setTimeout(warmOcr, OCR_CONFIG.performance.warmupFallbackDelayMs);
        }
    }
