...
export const Utils = {
    ...
    shouldWarmOcr() {
        ...
        if (OCR_CONFIG.warmup.slowEffectiveTypes.includes(connection?.effectiveType)) return false;
        if (navigator.deviceMemory && navigator.deviceMemory < OCR_CONFIG.warmup.lowDeviceMemoryGb) return false;
        return true;
    },
    ocrCanvasMaxSide() {
        return navigator.deviceMemory && navigator.deviceMemory < OCR_CONFIG.warmup.lowDeviceMemoryGb
            ? OCR_CONFIG.canvas.lowMemoryMaxSide
            : OCR_CONFIG.canvas.maxSide;
    },
