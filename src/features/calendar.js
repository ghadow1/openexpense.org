...
function getCalendarDensity(colEl) {
    ...
    const density = PLATFORM_CONFIG.calendarDensity;
    if (Utils.isMobile()) return 'mobile';
    if (colW > 0 && colW < density.compactMax) return 'compact';
    if (colW > 0 && colW < density.narrowMax) return 'narrow';
    if (colW > 0 && colW < density.tabletMax) return 'tablet';
    if (window.matchMedia(`(max-width: ${PLATFORM_CONFIG.breakpoints.tabletMax}px)`).matches) return 'tablet';
    return 'desktop';
}
