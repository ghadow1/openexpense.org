...
export const Receipt = {
    // @ocr-deps: lazy CDN imports keep the initial app bundle small. The
    // import map in index.html resolves peer deps for ppu-paddle-ocr.
    ...
    async ensureEngine(onProgress) {
        ...
        // @ocr-engine @perf: one shared service instance avoids re-downloading
        // models and keeps repeat scans fast on desktop and mobile browsers.
        Receipt._initPromise = (async () => {
            ...
            warm.width = warm.height = OCR_CONFIG.warmup.probeCanvasSize;
    ...
    // @ocr-pdf @platform: PDF.js is only loaded for PDF invoices, and native
    // text extraction skips OCR when the document already has readable text.
    async loadPdfJs() {
    ...
    async pdfToCanvasAndText(file, onProgress) {
        ...
        const scale = Math.min(
            ...
            OCR_CONFIG.image.maxSide / Math.max(baseViewport.width, baseViewport.height)
        );
    ...
    async ocrCanvas(service, canvas, onProgress) {
        ...
        // @ocr-pipeline: preserve OCR line geometry first; use flattened text as
        // a fallback for engines/browsers that return empty region groups.
    ...
    async decodeImage(file) {
        ...
        if (typeof createImageBitmap === 'function') {
            try {
                ...
                // Fall through to HTMLImageElement for Safari and HEIC paths.
            }
        }
        ...
        try {
            ...
            return { image: img, previewUrl: url, close: () => {} };
        } catch (err) {
    ...
    async fileToCanvas(file) {
        const decoded = await Receipt.decodeImage(file);
        try {
    ...
    async canvasToPreviewUrl(canvas) {
        const quality = OCR_CONFIG.image.previewQuality;
    ...
    // @ocr-pipeline @perf: normalize input dimensions so small receipts have
    // enough pixels for OCR while very large camera images stay memory-safe.
    ...
    showProgress() {
        ...
        backdrop.innerHTML = `
            <div class="modal-shell ocr-progress" role="status" aria-live="polite">
                ...
                <p class="ocr-progress-note">First scan downloads ${OCR_CONFIG.dependencies.engineLabel} models (PDF reader on demand), then caches locally.</p>
    ...
    parse(text, lines, confidence = 0) {
        // @ocr-parse: deterministic heuristics only. The scanner suggests
        // fields for human review and never saves without confirmation.
    ...
    showPreview(parsed, previewUrl) {
        // @ocr-ui: every scanned value lands in an editable review sheet before
        // saveExpense writes it to the encrypted ledger.
