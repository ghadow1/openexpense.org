...
export const Receipt = {
    ...
    bindIntentWarmup() {
        ...
        const warmOnIntent = (event) => {
            if (!event.target.closest('[data-action="scan-receipt"], .toolbar-scan-btn')) return;
            if (Utils.shouldWarmOcr()) Receipt.warmEngine();
        };
        document.addEventListener('pointerover', warmOnIntent, { passive: true });
        document.addEventListener('pointerdown', warmOnIntent, { passive: true, capture: true });
        document.addEventListener('touchstart', warmOnIntent, { passive: true, capture: true });
    ...
    async pdfToCanvasAndText(file, onProgress) {
        ...
        return {
            ...
            lines,
            previewUrl,
            hasExtractedText: text.trim().length >= OCR_CONFIG.pdf.minNativeTextChars
                || lines.length >= OCR_CONFIG.pdf.minNativeTextLines
        };
    },
    ...
    async fileToCanvas(file) {
        ...
        let decoded = null;
        try {
            decoded = await Utils.decodeImageFile(file);
